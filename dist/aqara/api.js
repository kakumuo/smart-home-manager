...
import { AqaraMotionSensor, MOTION_SENSOR_ILLUMINANCE_RID, MOTION_SENSOR_ILLUMINATION_RID, MOTION_SENSOR_MOTION_STATUS_RID } from "./devices.js";
import { generateSession } from "./aqaraCrypto.js";
export var AQARA_TARGET_COUNTRY = 'us';
const API_PATH = '/v3.0/open/api';
...
export const execEndpoint = (data, method, curSession) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        let responseData = "";
        let hostname = getDomainSDKURL(AQARA_TARGET_COUNTRY);
        let requestOptions = {
            method: method,
            hostname: hostname,
            path: API_PATH,
        };
        const session = generateSession(curSession);
        ...
        req.setHeader("Appid", session.appId);
        req.setHeader("Keyid", session.keyId);
        req.setHeader("Nonce", session.nonce);
        req.setHeader("Time", session.time);
        req.setHeader("Sign", session.sign);
        if (session.accessToken != null && session.accessToken != "" && session.accessToken != undefined) {
            req.setHeader("AccessToken", session.accessToken);
        }
...
export const getAuthCode = (emailAddress, session, accountType = 0) => __awaiter(void 0, void 0, void 0, function* () {
    let data = {
    ...
    const response = yield execEndpoint(data, 'post', session);
    response.session.authCode = response.responseData['result']['authCode'];
    return response.session;
});
export const getAccessToken = (emailAddress, session, accountType = 0) => __awaiter(void 0, void 0, void 0, function* () {
    let data = {
    ...
    console.log("get access token data: ", data);
    const response = yield execEndpoint(data, 'post', session);
    ...
    response.session.timestamp = Math.round(new Date().getTime());
    return response.session;
});
export const refreshToken = (session) => __awaiter(void 0, void 0, void 0, function* () {
    let data = {
    ...
    const response = yield execEndpoint(data, 'post', session);
    response.session.accessToken = response.responseData['accessToken'];
    response.session.refreshToken = response.responseData['refreshToken'];
    response.session.openId = response.responseData['openId'];
    response.session.expiresIn = response.responseData['expiresIn'];
    response.session.timestamp = Math.round(new Date().getTime());
    return response.session;
});
export const createAccount = (emailAddress, session) => __awaiter(void 0, void 0, void 0, function* () {
    let data = {
    ...
    if (response.responseData.code == 2016) {
        const updatedSession = yield getAuthCode(emailAddress, session, 2);
        targetSession = yield getAccessToken(emailAddress, updatedSession, 2);
    }
    else {
        response.session.accessToken = response.responseData['accessToken'];
        response.session.refreshToken = response.responseData['refreshToken'];
        ...
        response.session.expiresIn = response.responseData['expiresIn'];
        response.session.timestamp = Math.round(new Date().getTime());
        targetSession = response.session;
    }
    return targetSession;
});
export const getDevices = (session, deviceId = null) => __awaiter(void 0, void 0, void 0, function* () {
    const data = {
    ...
    response.responseData['result']['data'].forEach((deviceDetails) => {
        deviceList.push(createDevice(deviceDetails));
    });
    return deviceList;
});
export const updateDeviceState = (session, device) => __awaiter(void 0, void 0, void 0, function* () {
    let resourceIds;
    if (device instanceof AqaraMotionSensor) {
        resourceIds = [MOTION_SENSOR_ILLUMINANCE_RID, MOTION_SENSOR_ILLUMINATION_RID, MOTION_SENSOR_MOTION_STATUS_RID];
    }
    const data = {
    ...
    const response = yield execEndpoint(data, 'post', session);
    response.responseData['result'].forEach((resourceDetails) => {
        device.updateDeviceState(resourceDetails);
    });
    return device;
});
/* UITL */
export const createDevice = (deviceDetail) => {
    if (deviceDetail.model.indexOf("motion")) {
        return new AqaraMotionSensor(deviceDetail);
    }
    return null;
};
